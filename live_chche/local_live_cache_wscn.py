...
# 修改为本地服务器地址
url = "http://localhost:3002/v1/t2a_v2?GroupId=" + group_id
headers = {"Content-Type": "application/json", "Authorization": "Bearer " + api_key}
# 注意：本地服务器会接收但不会实际使用GroupId和Authorization
...
def build_tts_stream_body(text: str, language: str = "German", voice_id: str = "violet_de", lang: str = None) -> dict:
    ...
    body = json.dumps({
        ...
        "voice_setting": {
            "voice_id": "226869409853570",  # 修改为字符串类型，与浏览器请求保持一致
            "speed": 1.0,
            "vol": 1.0,
            "pitch": 0,
...
def contains_audio_data(larger_chunk, smaller_chunk):
    """
    检查较大的音频块是否包含较小的音频块

    参数:
    - larger_chunk: 较大的音频块
    - smaller_chunk: 较小的音频块

    返回:
    - 如果较大的块包含较小的块，返回True，否则返回False
    """
    # 如果较小的块比较大的块还大，肯定不包含
    if len(smaller_chunk) > len(larger_chunk):
        return False

    # 简单的启发式检查：在较大的块中查找较小块的前N个字节
    # 这不是完美的检测，但对于大多数情况应该足够了
    sample_size = min(1024, len(smaller_chunk))  # 取较小块的前1KB或全部
    sample = smaller_chunk[:sample_size]

    # 在较大的块中查找样本
    return sample in larger_chunk

def audio_play(audio_stream: Iterator[str]) -> bytes:
    """将16进制字符串流转换为二进制并播放"""
    print("开始流式播放...")
    audio_chunks = []  # 存储所有音频块
    buffer = b""  # 预缓冲区
    ...
    for chunk in audio_stream:
        if chunk is not None and chunk != '\n':
            try:
                ...
                audio_chunks.append(decoded_hex)
    ...
    if len(audio_chunks) == 1:
        print(f"只有一个音频块，大小: {len(audio_chunks[0])}字节")
        return audio_chunks[0]

    # 如果有多个音频块，使用第一个块（因为对于单词来说第一个块通常包含完整内容）
    if len(audio_chunks) >= 2:
        first_chunk = audio_chunks[0]
        print(f"使用第一个音频块，大小: {len(first_chunk)}字节")
        return first_chunk

    # 如果以上条件都不满足（理论上不会发生），则合并所有块
    combined_audio = b"".join(audio_chunks)
    print(f"合并所有音频块，总大小: {len(combined_audio)}字节")
